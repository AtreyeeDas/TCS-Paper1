...
def process_kaldi_directory(mucs_kaldi_dir: str, output_audio_dir: str):
    ...
    for rec_id, segs in tqdm(rec_to_segments.items(), desc="Processing Recordings"):
        ...
        # If wav.scp just has a filename, assume it's in the same folder or update path logic
        if not os.path.exists(audio_path):
            audio_path = os.path.join(mucs_kaldi_dir, audio_path)
    ...
    print(f"[✓] Slicing complete! All files saved to {output_audio_dir}")

if __name__ == "__main__":
    # Point this to the subfolder containing text, segments, wav.scp, etc.
    KALDI_DIR = "./path_to_your_mucs_transcript_subfolder" 
